...
def register(bot):
    ...
    bot.handlers['result-get'] = get_result


def ask_wolfram(message, bot: Leonard):
    bot.user_set(message.u_id, 'next_handler', 'result-get')
...
def get_result(message, bot: Leonard):
    ...
    if len(res) > 0 and not any(v is None for v in res):
        if res[0].text is not None:
            ...
            bot.telegram.send_message(message.u_id, 'Wolfram Alpha doesn\'t know anything about that 😢',
                                      reply_markup=reply_markup)
